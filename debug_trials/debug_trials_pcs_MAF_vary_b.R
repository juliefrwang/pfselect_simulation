...
sub_dir <- glue("simulation_res/seed_{seed}_b_{b}_rep_{num_replicate}_true_{num_true}_hete_{num_affected}_pcs_filter_MAF_{MAF}_vary_b")
...
snps <- snps[, apply(snps, 2, sum) > 5000 * 2 * MAF]
...
beta[eur_idx] <- sample(c(-b/2, b/2), length(eur_idx), replace = TRUE)
