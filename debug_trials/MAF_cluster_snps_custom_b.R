...
sub_dir <- glue("simulation_res/seed_{seed}_b_{b}_rep_{num_replicate}_true_{num_true}_hete_{num_affected}_pcs_filter_MAF_diff_ans_b_clust_snps_2")
...
# beta_matrix[, true_idx] <- sweep(beta_matrix[, true_idx], 1, ancestry[,'EUR'], "*")
...
my_function <- function(j) {
  ...
  write_json(results$W_statistic_matrix, json_file, auto_unbox = TRUE)
