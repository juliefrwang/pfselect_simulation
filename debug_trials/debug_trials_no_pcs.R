...
b <- as.numeric(args[1])
num_replicate <- as.numeric(args[2])
seed <- 500
num_true <- 30
num_affected <- 20
...
sub_dir <- glue("simulation_res/seed_{seed}_b_{b}_rep_{num_replicate}_true_{num_true}_hete_{num_affected}_no_pcs")
...
beta[true_idx] <- sample(c(-b, b), num_true, replace = TRUE)
...
for (i in 1:num_replicate){
  ...
  results <- get_importance_matrices(
    ...
    Z = matrix(numeric(nrow(snps))), # or other variables like 'pcs'
    y = true_y,
    n_folds = 5,
    FDR_rate = 0.1
  )
