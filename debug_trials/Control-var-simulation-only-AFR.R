...
sub_dir <- glue("simulation_res/seed_{seed}_b_{b}_FDR_{FDR_rate}_rep_{num_replicate}_true_{num_true}_hete_{num_affected}_exclRare_{excl_rare}_ctrlCorr_{ctrl_corr}_standardize_{standardize}_sampleSize_{sample_size}_{model}_ctrlAnsMaf_{ctrl_ans_maf}_only_homo_afr")
...
if (ctrl_ans_maf) {
  ...
  snp_high_maf_name <- colnames(for_frequency)[apply(for_frequency, 2, function(col) all(col > 0.1))]
  snps_to_choose <- snps[, snp_high_maf_name]
} else {
  snps_to_choose <- snps
}
