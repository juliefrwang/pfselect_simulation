...
sub_dir <- glue("simulation_res/seed_{seed}_b_{b}_FDR_{FDR_rate}_rep_{num_replicate}_true_{num_true}_hete_{num_affected}_exclRare_{excl_rare}_ctrlCorr_{ctrl_corr}_standardize_{standardize}_w_AMR_sampleSize_{sample_size}_model_{model}")
...
if (sample_size == "equal") {
  # Generate sample data with equal sample size in each ancestry backgroud
  snp_data <- snp_data %>%
