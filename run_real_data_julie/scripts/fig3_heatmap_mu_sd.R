...
categorize_snps_by_iqr <- function(scaled_selection_matrix) {
  # Compute standard deviation for each SNP
  # snp_std_dev <- apply(scaled_selection_matrix, 2, IQR, na.rm = TRUE)
  snp_std_dev <- apply(scaled_selection_matrix, 2, function(x) sd(x, na.rm = TRUE) / mean(x, na.rm = TRUE))
...
print_snp_iqr <- function(snp_categories, scaled_selection_matrix) {
  for (category in names(snp_categories)) {
    ...
    # snp_std_devs <- apply(scaled_selection_matrix[, snps, drop = FALSE], 2, IQR, na.rm = TRUE)
    snp_std_devs <- apply(scaled_selection_matrix[, snps, drop = FALSE], 2, function(x) sd(x, na.rm = TRUE) / mean(x, na.rm = TRUE))
...
load_directory = glue("~/Project/Knockoff/run_real_data_julie/run_standardize_FALSE_exclRare_TRUE_ctrlCorr_FALSE_0.9_fdr_0.2_model_Lasso_seed_400")
...
snp_categories <- categorize_snps_by_iqr(scaled_selection_matrix_filtered)
print_snp_iqr(snp_categories, scaled_selection_matrix_filtered)
# 
...
snp_categories <- categorize_snps_by_iqr(scaled_W_selected) # var calculated include negative W
print_snp_iqr(snp_categories, scaled_W_selected)
...
all_ranked_W_heterogeneity_plot_scaled <- plot_heatmap(
  ...
  ancestry = non_eur,
  ancestry_name = "non-EUR",
  category = "ranked_W_all_scaled_sd_mu",
  save_path = load_directory,
  max_non_zero = max_non_zero,
  min_non_zero = min_non_zero,
  w = 8,
